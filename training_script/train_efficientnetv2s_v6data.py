"""
VocabScan CP2 - EfficientNetV2-S training (dataset_v4)
========================================================
New script (no v3-data predecessor exists in this repo). Mirrors the same
"Option D" pattern as train_mobilenetv2_v4data.py / train_mobilenetv3_v4data.py:
two-stage transfer learning + class_weight to counter dataset_v4's class
imbalance (toothbrush: 102 images vs shoe: 299, ~2.9x ratio).

EfficientNetV2S ships with preprocessing BUILT INTO the model
(include_preprocessing=True, like MobileNetV3). It expects RAW 0-255 pixels -
no extra Rescaling layer needed.

EfficientNetV2S has 513 layers in its base (vs MobileNetV3Large's ~266).
Freezing the first 300 (~58%) mirrors the proportion frozen in the
MobileNetV3Large script (150/266 ~= 56%).

Option D tuning (same as the other two Keras scripts):
  Stage 1: epochs=30, lr=1e-3, patience=8
  Stage 2: epochs=20, lr=5e-5, patience=8
  Dropout=0.4, Batch=16, stronger augmentation

Run on HPC:
    python train_efficientnetv2s_v4data.py --data dataset_v4
"""
...
PATIENCE = 8
FREEZE_UP_TO = 300  # of 513 total layers in EfficientNetV2S
...
def main():
    ...
    try:
        ...
        ax.set_title("EfficientNetV2-S Confusion Matrix (test set, dataset_v4)")
