"""
VocabScan CP2 - YOLO11n-cls training (dataset_v6)
===================================================
...
Uses the ultralytics classification API. dataset_v6's existing
...
Run on HPC:
    ...
    python train_yolo11ncls_v6data.py --data dataset_v6 --no-weighted   # fallback
"""
...
def make_weighted_trainer():
    ...
    class WeightedClassificationTrainer(ClassificationTrainer):
        def get_dataloader(self, dataset_path, batch_size=16, rank=0, mode="train"):
            ...
            # Ultralytics' training loop expects InfiniteDataLoader.reset().
            # Keep the official loader type, but replace its underlying sampler
            # with the weighted sampler so class balancing still applies.
            batch_sampler = getattr(loader, "batch_sampler", None)
            if batch_sampler is None or not hasattr(batch_sampler, "sampler"):
                raise RuntimeError("Unexpected Ultralytics dataloader structure.")
            if not hasattr(batch_sampler.sampler, "sampler"):
                raise RuntimeError("Unexpected batch sampler structure.")
            batch_sampler.sampler.sampler = sampler
            loader.reset()
            return loader
